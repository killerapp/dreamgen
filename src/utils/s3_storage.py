...
class S3StorageManager:
    ...
    def __init__(
        ...
        # Configure S3 client
        self.s3_client = self._create_s3_client(endpoint_url, access_key, secret_key, region)
    ...
    def _create_s3_client(
        ...
        client_args = {"service_name": "s3", "config": config}
    ...
    async def upload_image(
        ...
        try:
            ...
            with open(image_path, "rb") as f:
                extra_args = {"Metadata": s3_metadata, "ContentType": "image/png"}

                if self.public_read:
                    extra_args["ACL"] = "public-read"

                self.s3_client.upload_fileobj(f, self.bucket_name, s3_key, ExtraArgs=extra_args)
    ...
    def _get_image_metadata(self, s3_key: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if "prompt" not in metadata:
                prompt_key = s3_key.replace(".png", ".txt")
                try:
                    prompt_obj = self.s3_client.get_object(Bucket=self.bucket_name, Key=prompt_key)
    ...
    def delete_image(self, s3_key: str):
        ...
        try:
            ...
            prompt_key = s3_key.replace(".png", ".txt")
            self.s3_client.delete_object(Bucket=self.bucket_name, Key=prompt_key)
            ...
            for page in page_iterator:
                ...
                for obj in page["Contents"]:
                    ...
                    if not local_path.exists() or local_path.stat().st_size != obj["Size"]:
                        ...
                        self.s3_client.download_file(self.bucket_name, obj["Key"], str(local_path))
